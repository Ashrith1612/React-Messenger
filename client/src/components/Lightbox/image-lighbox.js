...
class ImageLightbox extends Component {
  ...
  handleMouseDown(event) {
    if (
      ...
      ImageLightbox.isTargetMatchImage(event.target)
    ) {
      this.addPointer(ImageLightbox.parseMouseEvent(event));
  ...
  handleMouseMove(event) {
    if (this.shouldHandleEvent(SOURCE_MOUSE)) {
      this.multiPointerMove(event, [ImageLightbox.parseMouseEvent(event)]);
    }
  }

  handleMouseUp(event) {
    if (this.shouldHandleEvent(SOURCE_MOUSE)) {
      this.removePointer(ImageLightbox.parseMouseEvent(event));
  ...
  handlePointerEvent(event) {
    if (this.shouldHandleEvent(SOURCE_POINTER)) {
      switch (event.type) {
        case 'pointerdown':
          if (ImageLightbox.isTargetMatchImage(event.target)) {
            this.addPointer(ImageLightbox.parsePointerEvent(event));
            this.multiPointerStart(event);
          }
          break;
        case 'pointermove':
          this.multiPointerMove(event, [
            ImageLightbox.parsePointerEvent(event),
          ]);
          break;
        case 'pointerup':
        case 'pointercancel':
          this.removePointer(ImageLightbox.parsePointerEvent(event));
          this.multiPointerEnd(event);
          break;
        default:
          break;
  ...
  handleTouchStart(event) {
    if (
      ...
      [].forEach.call(event.changedTouches, eventTouch =>
        this.addPointer(ImageLightbox.parseTouchPointer(eventTouch))
      );
      this.multiPointerStart(event);
    }
  }
    ...
    if (this.shouldHandleEvent(SOURCE_TOUCH)) {
      this.multiPointerMove(
        ...
        [].map.call(event.changedTouches, eventTouch =>
          ImageLightbox.parseTouchPointer(eventTouch)
        )
      );
    }
  }
  ...
  handleTouchEnd(event) {
    if (this.shouldHandleEvent(SOURCE_TOUCH)) {
      [].map.call(event.changedTouches, touch =>
        this.removePointer(ImageLightbox.parseTouchPointer(touch))
      );
      this.multiPointerEnd(event);
    }
  }
